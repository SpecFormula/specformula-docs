...
└─────────────────────────────────────────────────────────────┘
          ...
          ▼                    ▼                    ▼
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
│  Entity Mapping │  │    API Spec     │  │   ISA Config    │
│                 │  │                 │  │                 │
│  特務 → agents  │  │  summary: 打卡   │  │  指令格式定義    │
│  打卡記錄 →     │  │  POST /punch    │  │  資料來源配置    │
│  punch_records  │  │  request/       │  │  時間格式設定    │
│                 │  │  response       │  │                 │
└─────────────────┘  └─────────────────┘  └─────────────────┘
          │                    │                    │
          ▼                    ▼                    ▼
...
│                      SpecFormula 引擎                        │
│  • 解析指令語法 → 識別操作類型                                │
│  • 查詢 Entity Mapping → 定位資料表                          │
│  • 查詢 API Spec → 建構 HTTP 請求                            │
│  • 執行操作並驗證結果                                         │
└─────────────────────────────────────────────────────────────┘
```
