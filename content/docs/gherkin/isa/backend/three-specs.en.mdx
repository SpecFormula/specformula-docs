...
```mermaid
flowchart TB
    subgraph Feature[".feature Test Files"]
    ...
    subgraph ISA["ISA Config (Configuration)"]
        ISA1["Instruction syntax definition"]
        ISA2["API configuration"]
        ISA3["Data source configuration"]
    end

    subgraph Engine["SpecFormula Engine"]
        E1["Parse instruction syntax → Identify instruction type"]
...
- Whether tables in Entity Spec exist in DDL
- Whether fields used in .feature are defined in Entity Spec and API Spec
