...
flowchart TB
    ...
    SPEC -->|sync-global-features.sh| A0["implement Step Defs per vocab.md"]
    A0 --> A1["Java (RI) BDD"]
    ...
    A3 --> BV
    SPEC -->|sync-global-features.sh| B0["implement Step Defs per vocab.md"]
    B0 --> B1["other languages BDD"]
    B1 --> BV
```

**① SSOT update** — change `specs/` (ADR + `specs/features/` executable specs); `scripts/sync-global-features.sh <lang>` + terminology mapping mirrors it into each language.
