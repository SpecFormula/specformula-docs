...
flowchart TB
    ...
    SPEC -->|sync-global-features.py| A0["依 vocab.md 建立 Step Def"]
    A0 --> A1["Java（RI）BDD"]
    A1 --> A2{"新功能 / 行為？"}
    A2 -->|是| A3["更新 benchmark domain（E2E 情境）"]
    A2 -->|否| BV["benchmark E2E 驗收<br/>跨語言行為一致"]
    A3 --> BV
    SPEC -->|sync-global-features.py| B0["依 vocab.md 建立 Step Def"]
    B0 --> B1["其他語言 BDD"]
    B1 --> BV
```

**① SSOT 更新** — 改 `specs/`（ADR + `specs/features/` 可執行規格），`scripts/sync-global-features.py <lang>` + terminology mapping 鏡射到各語言。
...
| `specformula-lsp-plugin/` | LSP 整合方向，目前不作為主要貢獻入口 |
